...
export interface LessonContent {
  ...
  qa: { q: string; meta: string; a: string }[]
}

export const COURSE_DATA: Record<number, CourseData> = {
  1: {
    id: 1,
    icon: '💻',
...
export const LESSON_CONTENT: Record<string, LessonContent> = {
  l1: {
    ...
    points: [
      ...
      'Download all required tools and resources',
      'Join the course community forum',
      'Set your learning schedule for best results',
    ],
    resources: [
  ...
  l10: {
    ...
    points: [
      ...
      'Nest conditionals properly without spaghetti code',
    ],
    resources: [
...
export function getAllLessons(courseId: number | string): (Lesson & { moduleTitle: string; moduleId: string })[] {
  const course = COURSE_DATA[Number(courseId)]
  if (!course) return []
...
export function getLessonById(courseId: number | string, lessonId: string) {
  return getAllLessons(courseId).find(l => l.id === lessonId)
}

export function getNextLesson(courseId: number | string, lessonId: string) {
  const all = getAllLessons(courseId)
  const idx = all.findIndex(l => l.id === lessonId)
  return idx >= 0 && idx < all.length - 1 ? all[idx + 1] : null
}

export function getPrevLesson(courseId: number | string, lessonId: string) {
  const all = getAllLessons(courseId)
  const idx = all.findIndex(l => l.id === lessonId)
